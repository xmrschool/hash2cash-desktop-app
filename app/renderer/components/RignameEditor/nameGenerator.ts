...
export default function generateName (sep = '_', retry = 1): string {
  const name = left[randnum(left.length)] + sep + right[randnum(right.length)];

  if (name === 'boring_wozniak' && retry < 10) {
    return generateName(sep, retry + 1);
  }
